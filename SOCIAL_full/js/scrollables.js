/* ═══════════════════════════════════════════════════════════════
   ...
   Called from: _onPageLoad('scrollables'), pages/scrollables.html.
   ═══════════════════════════════════════════════════════════════ */
...
(function () {
  ...
  var sorted = embeds.slice().sort(function (a, b) {
    return (
      (parseInt(b.dataset.videoId || '0', 10) || 0) - (parseInt(a.dataset.videoId || '0', 10) || 0)
    );
  });
  var needsReorder = sorted.some(function (el, i) {
    return el !== embeds[i];
  });
  if (needsReorder)
...
(function () {
  ...
  setTimeout(async function () {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      overlay.appendChild(playDiv);
      wrapper.appendChild(img);
      wrapper.appendChild(overlay);

      /* Try to load thumbnail from TikTok CDN, fall back to initials */
      (function (imgEl, itemData) {
        var endpoints = [
        ...
        imgEl.addEventListener('error', function () {
          ...
          imgEl.remove();
          wrapper.insertBefore(fallback, overlay);
        });
      })(img, item);
