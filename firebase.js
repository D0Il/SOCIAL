  ...
  try {
    ...
    function handleSiteConfig(data) {
      ...
      if (data.insta_posts && data.insta_posts.length) {
        var validPosts = data.insta_posts.filter(function (post) { return post.src && post.src.trim(); });
        var grid = validPosts.length ? document.getElementById('grid-gallery') : null;
        if (grid) {
