...
!(function () {
  ...
  ((window.showPage = function (t) {
    ...
    document.addEventListener(
      'click',
      function (t) {
        t.target.closest('.panties-widget') &&
          o({
            type: 'link_click',
            dest: 'onlyfans',
          });
  ...
  function n() {
    ...
    e &&
      ...
      (window.innerWidth <= 768
        ? p &&
          p.parentNode &&
          e.parentNode !== p.parentNode &&
          ((o = e.parentNode), (r = e.nextSibling), p.parentNode.insertBefore(e, p.nextSibling))
        : o && e.parentNode !== o && o.insertBefore(e, r || null));
  }
...
!(function () {
  ...
  function ensureWidgetRow() {
    ...
    row ||
      ...
      (row.id = 'mobile-profile-widget-row'),
      (row.className = 'mobile-profile-widget-row'),
      host.insertBefore(row, lvl));
    row.appendChild(lvl);
...
/* ── Talk page: mobile tab sync (moves sidebar headers into timeline tabs on narrow screens) ── */
!(function () {
  var moved = false;
  var items = [];
  ...
  function moveTalkButtons() {
    ...
    var headers = Array.prototype.slice
      .call(rightCol.querySelectorAll('.sidebar-section-header'))
      .slice(0, 2);
...
!(function () {
  ...
  if (_ss)
    window.switchToSearch = function (q, t) {
